...
public class AudioManager : MonoBehaviour
{
    ...
    // Support looping sounds for a set code duration
    // Where would this be needed?
    // Imagine this scenario:
        // Player picks up a Shield Power-up.
        // While the shield is active, a “humming” sound should play continuously.
        // When the shield ends after, say, 5 seconds → the sound should stop automatically.
    // Useful for?
        // For temporary power-ups (shield, magnet, double-points, etc.).
        // For timed events(e.g., danger alarm that lasts only until a countdown finishes).
        // It prevents you from having to manually stop the sound later.
    public IEnumerator PlaySoundLoopForSpecificDuration(string name, float duration)
    {
        if (soundsDict.TryGetValue(name, out Sounds soundFound))
        {
            sfxSource.clip = soundFound.clip;
            sfxSource.volume = soundFound.volume;
            sfxSource.loop = true;
            sfxSource.Play();
